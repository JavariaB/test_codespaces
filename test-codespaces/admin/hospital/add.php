...
try {
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if (!isset($_POST['bed_name']) && empty($_POST['bed_name'])) {
		...
		if (isset($_POST['bed_name']) && !empty($_POST['bed_name'])) {
			...
			$result = $conn->query(sprintf('SELECT id FROM beds WHERE bed_name = "%s"', $_POST['bed_name']));

			if (is_object($result) && $result->num_rows > 0) {
				$errors['bed_name'] = 'This bed name has already registered.';
			}
		} else {
			$errors['bed_name'] = 'Bed name is required';
		}
		...
		if (empty($errors)) {
			...
			updateOrCreateHospitalBed($_SESSION['id'], $_POST['bed_name'], $_POST['is_available']);

			$success = 'New bed details added successfully.';
		}
	}
} catch (Exception $e) {
	$error = $e->getMessage();
...
<head>
	...
	<title>KrankenCare - Add Bed</title>
						...
						<div class="col-lg-12 col-md-12 mb-5">
							<div class="blog-item">
								<div class="blog-item-content">
									<h2 class="mb-3">Add New Bed</h2>

									<p class="mb-4">You can use this section to create a new bed record.</p>
									...
									<form action="" method="post">
										...
										<div class="row">
											...
											<div class="col-9">
												...
												<input name="bed_name" id="bed_name" type="text"  class="form-control <?= empty($errors['bed_name']) ?: 'invalid-input' ?>" value="<?= isset($_POST['bed_name']) && $has_errors ? $_POST['bed_name'] : '' ?>">
													<?php if (!empty($errors['bed_name'])) : ?>
														<small class="text-danger"><?= $errors['bed_name'] ?></small>
													<?php endif; ?>
												</div>
											</div>
										</div>
										...
										<div class="row">
											...
											<div class="col-9">
												<div class="form-group">
													<select name="is_available" id="is_available" class="form-control <?= empty($errors['is_available']) ?: 'invalid-input' ?>" placeholder="Please select a is_available." value="<?= isset($_POST['is_available']) && $has_errors ? $_POST['is_available'] : '' ?>">
										...
										<div class="row">
											...
											<div class="col-9">
												<div>
													<input class="btn btn-main btn-round-full mt-2" name="submit" type="submit" Style="background-color: #005073; border-color: #005073" value="Add Record"></input>
												</div>
											</div>
										</div>
									</form>
