...
try {
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if (empty($errors)) {
			...
			$sql = $conn->prepare('INSERT INTO users (name, email, password, role, postal_code) VALUES (?, ?, ?, ?, ?)');

			if (!$sql) throw new Exception($conn->error);

			$hashed_password = password_hash($_POST['password'], PASSWORD_DEFAULT);

			$sql->bind_param('sssss', $_POST['name'], $_POST['email'], $hashed_password, $_POST['role'], $_POST['postal_code']);
				...
				<div class="col-lg-8 col-md-8 col-sm-8">
					<form id="registration-form" class="contact__form" method="post" action="">
						<div class="form-group">
							...
							<input name="name" id="name" type="text" placeholder="Enter your hospital's name." class="form-control <?= empty($errors['name']) ?: 'invalid-input' ?>" value="<?= isset($_POST['name']) && $has_errors ? $_POST['name'] : '' ?>">
						...
						<div class="form-group">
							...
							<input name="postal_code" id="postal_code" type="text" placeholder="Enter your Postal Code" class="form-control <?= empty($errors['postal_code']) ?: 'invalid-input' ?>" value="<?= isset($_POST['postal_code']) && $has_errors ? $_POST['postal_code'] : '' ?>">
